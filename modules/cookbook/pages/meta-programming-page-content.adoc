...
The right starting point is the JavaDoc for the method of `TapestryModule` that contributes the base set: javadoc:org.apache.tapestry5.services.TapestryModule[label="contributeMarkupRenderer()"].
...
Before Tapestry 5.2 that was a small challenge, but 5.2 adds a method to javadoc:org.apache.tapestry5.services.RequestGlobals[label="RequestGlobals.getActivePageName()"] for this exact purpose.
