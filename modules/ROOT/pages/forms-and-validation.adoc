...
The Login page template below contains a minimal amount of Tapestry instrumentation and references some of the https://getbootstrap.com/[Bootstrap CSS] classes (Bootstrap is automatically integrated into each page by default, starting with Tapestry 5.4).
...
|checked (Since 5.4.5)
|boolean
|Makes sure that the boolean is true (checkbox is checked)
|`<t:Checkbox value="value" validate="checked" />`

|unchecked (Since 5.4.5)
|boolean
|Makes sure that the boolean is false (checkbox is unchecked)
...
TIP: In versions of Tapestry prior to 5.4, a form with validation errors would result in a redirect response to the client; often, temporary server-side data (such as the userName field) would be lost. Starting in 5.4, submitting a form with validation errors results in the new page being rendered in the same request as the form submission.
